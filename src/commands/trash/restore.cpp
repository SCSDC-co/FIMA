 ...
 *
 * The implementation of the `trash restore` subcommand
 *
...
restore(const std::vector<std::string>& ids)
{
    ...
    for (const auto& id : ids) {
        ...
        if (it != map_id_path.end()) {
            ...
            std::filesystem::rename(file_path, original_path);
            std::filesystem::remove(path);

            std::cout << original_path.string() << fima::theme::theme.primary << " restored."
