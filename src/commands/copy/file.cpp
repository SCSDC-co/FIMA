...
file(const fs::path source, const fs::path destination)
{
    if (!fs::is_regular_file(source)) {
        fima::helpers::log(
          fima::helpers::logger_type::ERROR,
    ...
    try {
        fs::copy_file(
          source, destination, fs::copy_options::overwrite_existing);

        std::clog << fima::colors::GREEN << "File " << fima::colors::RESET
                  << source.string() << fima::colors::GREEN << " copied to "
                  << fima::colors::RESET << destination.string() << '\n';
    } catch (const std::exception& ex) {
        fima::helpers::log(
          fima::helpers::logger_type::ERROR, "Failed to copy the file: ", "");

        fima::helpers::log(
          fima::helpers::logger_type::ERROR, "  Source file: ", source);
        fima::helpers::log(
